...
if [ -n "${SSHFS_HOST+set}" ]; then
  ...
  if [ -n "${SSHFS_NAMESPACE+set}" ]; then
    ...
    mv /home/vcap/app/htdocs/restaurant-7.x-1.0-rc2/sites /home/vcap/app/htdocs/restaurant-7.x-1.0-rc2/mirage
    echo -e "${delivery}${Yellow}  Creating Symlink between Drupal Sites folder and mounted Domain Namespace location ..."
    ln -s /home/vcap/misc/${SSHFS_NAMESPACE}/sites /home/vcap/app/htdocs/restaurant-7.x-1.0-rc2
    # Comment:  cp is too slow, even with 180 sec extended health check
    # cp -R /home/vcap/app/htdocs/restaurant-7.x-1.0-rc2/mirage/. /home/vcap/app/htdocs/restaurant-7.x-1.0-rc2/sites
    # Comment:  scp was too slow, even with 180 sec extended health check
    # scp -r /home/vcap/app/htdocs/restaurant-7.x-1.0-rc2/mirage/. /home/vcap/app/htdocs/restaurant-7.x-1.0-rc2/sites
    ...
    if [ -f "/home/vcap/misc/${SSHFS_NAMESPACE}/sites/default/settings.php" ]; then
      ...
      tar -C /home/vcap/app/htdocs/restaurant-7.x-1.0-rc2/mirage -jcf - ./ | ssh -i /home/vcap/app/.profile.d/id_rsa -o UserKnownHostsFile=/home/vcap/app/.profile.d/known_hosts ${SSHFS_USER}@${SSHFS_HOST} "tar -C/home/paramount/${SSHFS_NAMESPACE}/sites -ojxf -"
    ...
    rm -rf /home/vcap/app/htdocs/restaurant-7.x-1.0-rc2/mirage
  else
